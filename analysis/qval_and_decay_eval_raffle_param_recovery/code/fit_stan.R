...
model_file <- file.path(project_root, "models", "qval_and_decay_eval_raffle", "qval_and_decay_eval_raffle.stan")

Nsubjects    <- max(df$subject)
Narms_fit    <- max(df$ch_card)
Nraffle_fit  <- length(df$offered_arms[[1]])
offered_arms <- do.call(rbind, df$offered_arms)
