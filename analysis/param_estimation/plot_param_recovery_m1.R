...
posterior_panel <- function(draws_vec, xlabel, is_effect = FALSE) {
  ...
  p <- ggplot(df, aes(x = x)) +
    ...
    annotate("text", x = med, y = 1.05, label = ann_label,
             hjust = 0.5, vjust = 0, size = 3.2, colour = GREY40) +
    labs(x = xlabel) +
...
pA <- posterior_panel(draws_group$alpha_pop, "alpha  (learning rate)",      is_effect = FALSE)
pB <- posterior_panel(draws_group$beta_pop,  "beta  (inverse temperature)", is_effect = TRUE)
pC <- posterior_panel(draws_group$kappa_pop, "kappa  (perseveration)",      is_effect = TRUE)
pD <- posterior_panel(draws_group$delta_pop, "delta  (decay rate)",         is_effect = FALSE)

fig <- (pA | pB | pC | pD) +
  plot_annotation(
    title      = "M1 population posteriors",
    tag_levels = "A"
  ) &
