...
s_alpha <- subject_panel("alpha_ab", "alpha")
s_beta  <- subject_panel("beta_ab",  "beta")

# ── combine ───────────────────────────────────────────────────────────────────
top_row    <- p_alpha | p_beta
bottom_row <- s_alpha | s_beta

fig <- top_row / bottom_row +
  plot_annotation(tag_levels = "A") &
  theme(plot.tag = element_text(face = "bold", size = 14))

ggsave(file.path(OUT_DIR, "param_estimation_ab.png"), fig,
       width = 7, height = 7, dpi = 150, bg = "white")
cat("Saved: figures/param_estimation_ab.png\n")
