...
p_alpha <- posterior_panel(draws_group$alpha_pop, "alpha  (learning rate)",           is_effect = FALSE, fill_col = GREY80)
p_beta  <- posterior_panel(draws_group$beta_pop,  "beta  (inverse temperature)",      is_effect = TRUE,  fill_col = GREY80)
...
s_alpha <- subject_panel("alpha_abdr", "alpha")
s_beta  <- subject_panel("beta_abdr",  "beta")
s_delta <- subject_panel("delta_abdr", "delta")
s_rho   <- subject_panel("rho_abdr",   "rho")

# ── combine ───────────────────────────────────────────────────────────────────
top_row    <- p_alpha | p_beta | p_delta | p_rho
bottom_row <- s_alpha | s_beta | s_delta | s_rho

fig <- top_row / bottom_row +
  plot_annotation(tag_levels = "A") &
  theme(plot.tag = element_text(face = "bold", size = 14))

ggsave(file.path(OUT_DIR, "param_estimation_abdr.png"), fig,
       width = 12, height = 7, dpi = 150, bg = "white")
cat("Saved: figures/param_estimation_abdr.png\n")
