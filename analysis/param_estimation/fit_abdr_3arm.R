...
MODEL_FILE <- "../../models/abdr_3arm/abdr_3arm.stan"
...
print(fit, pars = c("alpha_pop", "beta_pop", "delta_pop", "rho_pop"))
...
draws_group <- tibble(
  ...
  delta_pop = draws$delta_pop,
  rho_pop   = draws$rho_pop
)
write_csv(draws_group, file.path(DRAWS_DIR, "posterior_draws_group.csv"))

#### SAVE SUBJECT DRAWS ####
sbj_param_map <- list(
  alpha = draws$alpha_sbj,
  beta  = draws$beta_sbj,
...
#### SAVE TRIAL-LEVEL Q AND E MEDIANS ####
...
qe_trial_df <- df |>
  ...
  mutate(
    Q1 = Q_median[, 1], Q2 = Q_median[, 2], Q3 = Q_median[, 3],
    E1 = E_median[, 1], E2 = E_median[, 2], E3 = E_median[, 3]
  )
write_csv(qe_trial_df, file.path(DRAWS_DIR, "qe_trial_median.csv"))
...
cat("Saved Q/E trial median ->", file.path(DRAWS_DIR, "qe_trial_median.csv"), "\n")
