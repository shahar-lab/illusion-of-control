...
model_file <- file.path(project_root, "models", "<model_name>", "<model_name>.stan")

Nsubjects <- max(df$subject)
Narms_fit <- max(df$ch_card)   # data-driven — never hardcode Narms here
...
fit <- model$sample(
  ...
  chains          = 4,
  parallel_chains = 4,
  iter_warmup     = 2000,    # sanity run: 100-500; full run: 2000 (see RUN_CONVENTIONS.md)
  iter_sampling   = 3000,    # sanity run: 100-500; full run: 3000
  refresh         = 200
)
...
# Subject-level posterior medians (read downstream by plot_recovery.R / summary_table.R).
# Saved here, not in plot_recovery.R, so downstream scripts never need to reload fit.rds.
...
# Population-level parameter posterior draws (read downstream by plot_population_posteriors.R)
