...
sim.block = function(subject,parameters,cfg){ 
  ...
  for (block in 1:Nblocks){
    ...
    for (trial in 1:Ntrials){
      ...
      # add perseveration bias on the previously chosen option
      Qnet = rep(NA, length(options))
      for (i in seq_along(options)){
        Qnet[i] = beta * Q_cards_offered[i] + if_else(!is.na(prev_choice) & options[i] == prev_choice, kappa, 0)
      }
